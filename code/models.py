...
def define_CNN_model(utterance_representations_full, num_filters=300, vector_dimension=300,
    ...
    """
    filter_sizes = [1, 2, 3]
    with tf.name_scope("user_utterance_cnn") as scope:
        ...
        hidden_representation = tf.identity(hidden_representation, name=scope)
...
def model_definition(vector_dimension, label_count, slot_vectors, value_vectors, use_delex_features=False,
    ...
    print
    "Hidden layer size:", hidden_units_1, "Label Size:", label_size, "Use Softmax:", use_softmax, "Use Delex Features:", use_delex_features

    utterance_representations_full = tf.placeholder(tf.float32, [None, 40,
    ...
    if use_softmax:
        y_combine = tf.identity(y_combine, name="y_combine")
    ...
    return keep_prob, \
           ...
           utterance_representations_delex, \
           system_act_slots, \
